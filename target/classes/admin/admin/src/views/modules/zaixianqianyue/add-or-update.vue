      ...
      <el-row>
        ...
        <el-form-item class="input" v-if="type!='info'"  label="租赁金额" prop="zulinjine">
          <el-input v-model="ruleForm.zulinjine" 
              placeholder="租赁金额" clearable  :readonly="ro.zulinjine"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="租赁金额" prop="zulinjine">
              <el-input v-model="ruleForm.zulinjine" 
...
export default {
  ...
  methods: {
    ...
    onSubmit() {
        ...
        if (valid) {
          this.$http({
            url: `zaixianqianyue/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.zaixianqianyueCrossAddOrUpdateFlag = false;
                  this.parent.search();
